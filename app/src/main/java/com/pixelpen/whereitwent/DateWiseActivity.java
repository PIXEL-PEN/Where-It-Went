...
public class DateWiseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Load expenses for ALL dates in DB, grouped by the saved date string
        List<Expense> expenses = ExpenseDatabase
                .getDatabase(this)
                .expenseDao()
                .getAll(); // we’ll render them under date banners, same as before

        expensesContainer.removeAllViews();

        // We’ll add a banner per day we encounter, summing as we go.
        // Since original layout was simple, we iterate in DB order.
        String currentDate = null;
        double runningTotalForDay = 0.0;

        for (int i = 0; i < expenses.size(); i++) {
            Expense e = expenses.get(i);

            // When date changes, if not first section, write the TOTAL row for the previous date
            if (currentDate != null && !currentDate.equals(e.date)) {
                addTotalRow(runningTotalForDay, symbol);
                runningTotalForDay = 0.0;
            }

            // New date section banner
            if (currentDate == null || !currentDate.equals(e.date)) {
                currentDate = e.date;
                addDateBanner(formatFullDate(currentDate));
            }

            // Row for this expense
            ...
            // Click → details dialog (shows tag on category line)
            ...
            runningTotalForDay += e.amount;

            // If this is the last item, flush total row
    ...
    private void addDateBanner(String bannerText) {
